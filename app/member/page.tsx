...
type FXBaseProps = {
  ...
  rainbowRing?: boolean;
  durationMs?: number;
  chestSrc: string; // /fantasy/chest/chest_closed.svg or chest_open.svg
  showBadge?: string; // /fantasy/icons/badge_xxx.svg
};
...
function FXOverlay({
  ...
  return (
    <div className="fixed inset-0 z-[60] pointer-events-none">
      {/* subtle fantasy background (fallback to gradient plus bg.svg) */}
      <div
      ...
      <style jsx global>{`
        ...
        .animate-fx-fade { animation: fx-fade 1.7s ease both }
...
function badgeSrcFromCode(code: string) {
  ...
  // SPECIAL_LEGENDARY -> special_legendary
...
function OpenRewardFX({
  ...
  const pal = rarityPalette[toRarity(rarityCode)];
  const value =
    rewardType === "CASH" ? `+${formatIDR(rewardAmount)} saldo` : rewardLabel;
  const isRainbow = toRarity(rarityCode) === "SPECIAL_LEGENDARY";
  return (
    <FXOverlay
      open={open}
...
export default function MemberHomePage() {
  ...
  // last sections (optional display)
  ...
  useEffect(() => {
    // precreate audio tags (will only play if soundOn)
  ...
  const handlePurchase = async (tier: 1 | 2 | 3) => {
    ...
    // Update credit UI (prefer credits_after)
  ...
  const handleOpenBox = async (box: InventoryBox) => {
    ...
    // FX reward
    play(sfxRumble);
    setTimeout(() => play(sfxReveal), 250);
    if (result.reward_type === "CASH") {
      setTimeout(() => play(sfxCoin), 520);
    }
    setFxOpen({
              ...
              <button
                ...
                className={`text-xs rounded-md border px-2 py-1 ${
                  soundOn
                    ...
                    : "border-slate-600/60 text-slate-300 hover:bg-slate-800/50"
                }`}
          ...
          <div className="mt-6 rounded-2xl border border-slate-700/70 bg-slate-950/70 p-4">
              ...
              <div className="text-xs text-slate-300">
                ...
                <div className="mt-1 text-slate-400">
                  ...
                  <span className="font-medium">
                    {formatIDR(lastPurchase.credits_before)}
                  </span>{" "}
                  • setelah beli:{" "}
                  <span className="font-medium">
                    {formatIDR(lastPurchase.credits_after)}
                  </span>
                </div>
            ...
            {lastOpened && (
              <>
                ...
                <div className="text-xs text-slate-300">
                  Rarity {lastOpened.rarity_name} • Hadiah{" "}
                  <span className="font-semibold">{lastOpened.reward_label}</span>
                  {lastOpened.reward_type === "CASH" && (
                    <> (+{formatIDR(lastOpened.reward_amount)} saldo)</>
                  )}
                </div>
              </>
            )}
