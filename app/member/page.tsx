...
// Format nama rarity biar enak dibaca
...
export default function MemberHomePage() {
  ...
  async function fetchAllForMember(uid: string) {
    ...
    // Inventory: box PURCHASED & belum expired + ambil rarity
  ...
  // Beli box: panggil RPC, lalu ambil rarity dari box_transactions by transaction_id
  async function handlePurchase(tier: number) {
    ...
    try {
      ...
      const rpcRow = Array.isArray(data) ? (data as any[])[0] : (data as any);
      const transactionId: string | undefined = rpcRow?.transaction_id;
      const finalTier = rpcRow?.credit_tier ?? tier;

      let rarityCode: string | null = null;
      let rarityName = "";
      ...
      if (transactionId) {
        const { data: txRow, error: txErr } = await supabase
          ...
          .select("rarity")
          .eq("id", transactionId)
          .maybeSingle<{ rarity: string | null }>();

        if (!txErr && txRow) {
          rarityCode = txRow.rarity;
          rarityName = formatRarityName(txRow.rarity);
        } else if (txErr) {
          console.error("Error fetching rarity after purchase:", txErr);
        }
      }

      // refresh semua data (profile + inventory + last opened)
      if (memberId) {
